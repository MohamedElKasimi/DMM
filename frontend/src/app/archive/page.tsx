...
export default function Archive() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>(new Date().toISOString().substr(0, 7)); // YYYY-MM format
  const [appliedDate, setAppliedDate] = useState<string>(selectedDate);
  ...
  const fetchKPIs = async () => {
    try {
      ...
      const response = await axios.get<KPIData[]>(`http://localhost:8000/performances/performanceData/?date=${appliedDate}`, {
        headers: {
